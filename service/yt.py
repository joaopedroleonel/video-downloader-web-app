...
class Yt:
    ...
    def download(self, playlist, type, url, id, session):
        self.r.hset(id, mapping={'status': 'Vídeo em processamento.', 'session': session})
        ...
        def progress_hook(d):
            if d['status'] == 'downloading':
                ...
                fmt = info.get('format', '')
                self.r.hset(id, mapping={'status': f'{title}[{fmt}] foi processado com sucesso.', 'session': session})
        ...
        if playlist == False:
            ...
            with YoutubeDL(opts) as ydl:
                ...
                self.r.hset(id, mapping={'status': 'O download do arquivo será iniciado em instantes.', 'session': session})   
                return True
        else:
            opts = ydl_opts_video if type == 1 else ydl_opts_audio
        ...
        self.r.hset(id, mapping={'status': 'O download do vídeo não pôde ser realizado.', 'session': session})
        return False
